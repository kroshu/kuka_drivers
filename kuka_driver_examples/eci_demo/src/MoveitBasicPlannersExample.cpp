...
int main(int argc, char * argv[])
{
  ...
  addCollisionBox(Eigen::Vector3d(0.25, -0.075, 0.675), Eigen::Vector3d(0.1, 0.4, 0.1));
  moveit_visual_tools.trigger();
  moveit_visual_tools.prompt("Press 'Next' in the RvizVisualToolsGui window to execute");

  // Try moving back with Pilz LIN
  planned_trajectory = planToPoint(standing_pose, "pilz_industrial_motion_planner", "LIN");
  if (planned_trajectory != nullptr) {
    draw_trajectory_tool_path(*planned_trajectory);
  } else {
    draw_title("Failed planning with Pilz LIN");
  }
  moveit_visual_tools.trigger();
  moveit_visual_tools.prompt("Press 'Next' in the RvizVisualToolsGui window to execute");

  // Try moving back with Pilz PTP
  planned_trajectory = planToPoint(standing_pose, "pilz_industrial_motion_planner", "PTP");
  if (planned_trajectory != nullptr) {
    draw_trajectory_tool_path(*planned_trajectory);
  } else {
    draw_title("Failed planning with Pilz PTP");
  }
  moveit_visual_tools.trigger();
  moveit_visual_tools.prompt("Press 'Next' in the RvizVisualToolsGui window to execute");
