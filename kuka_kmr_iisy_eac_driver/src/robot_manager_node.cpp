...
#include "kuka_kmr_iisy_eac_driver/kmr_robot_manager_node.hpp"
...
namespace kuka_eac
{
MobileRobotManagerNode::MobileRobotManagerNode() :  RobotManagerNodeBase()
{
  
  // Register parameters
    this->registerParameter<std::string>(
    "cartesian_pose_broadcaster_name", "cartesian_pose_broadcaster", 
  ...
  this->registerStaticParameter<std::string>(
    "robot_model", "kmr_iisy", kuka_drivers_core::ParameterSetAccessRights{false, false},
...
MobileRobotManagerNode::on_activate(const rclcpp_lifecycle::State &)
{
  ...
  controllers.push_back(kuka_drivers_core::CARTESIAN_POSE_BROADCASTER);
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn
MobileRobotManagerNode::on_deactivate(const rclcpp_lifecycle::State &)
{
...
bool MobileRobotManagerNode::onControlModeChangeRequest(int control_mode)
{
  ...
  if (control_mode != static_cast<int>(kuka_drivers_core::ControlMode::CARTESIAN_VELOCITY_CONTROL))
  {
...
int main(int argc, char * argv[])
{
  ...
  auto node = std::make_shared<kuka_eac::MobileRobotManagerNode>();
