...
public:
      ...
      Node("LifecycleClientTest")
  {
    ...
    client_ = this->create_client<lifecycle_msgs::srv::ChangeState>("/lc_talker/change_state",
                                                                    qos.get_rmw_qos_profile(), service_cbg_);
  ...
  virtual rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn configure()
  {
    ...
    auto future_result = client_->async_send_request(
        request, [this](std::shared_future<lifecycle_msgs::srv::ChangeState::Response::SharedPtr>)
  ...
  unsigned int get_state(std::chrono::seconds time_out)
  {
    ...
    {
      RCLCPP_ERROR(get_logger(), "Service %s is not available.", getter_client_->get_service_name());
