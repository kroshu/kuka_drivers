...
public:
    ...
    Node("LifecycleClientTest")
  {
    ...
    qos.reliable();
    service_cbg_ = this->create_callback_group(rclcpp::callback_group::CallbackGroupType::MutuallyExclusive);
    topic_cbg_ = this->create_callback_group(rclcpp::callback_group::CallbackGroupType::MutuallyExclusive);
    auto sub_opt = rclcpp::SubscriptionOptions();
    sub_opt.callback_group = topic_cbg_;
    client_ = this->create_client<lifecycle_msgs::srv::ChangeState>("/lc_talker/change_state", qos.get_rmw_qos_profile(), service_cbg_);
    getter_client_ = this->create_client<lifecycle_msgs::srv::GetState>("/lc_talker/get_state");
    trigger_ = this->create_subscription<std_msgs::msg::Bool>("change_state",qos,
  ...
  configure(){
    ...
    auto future_result = client_->async_send_request(request, [this](std::shared_future<lifecycle_msgs::srv::ChangeState::Response::SharedPtr>){RCLCPP_INFO(get_logger(), "callback called");});
  ...
  unsigned int
  get_state(std::chrono::seconds time_out)
  {
    ...
    if (future_result.get()) {
      RCLCPP_INFO(get_logger(), "current state %s.",future_result.get()->current_state.label.c_str());
      return future_result.get()->current_state.id;
    } else {
      RCLCPP_ERROR(
        get_logger(), "Failed to get current state");
      return lifecycle_msgs::msg::State::PRIMARY_STATE_UNKNOWN;
    }
  }
private:
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::executors::MultiThreadedExecutor executor(rclcpp::executor::create_default_executor_arguments(), 2);
