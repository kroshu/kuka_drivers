...
TEST_F(EncodeTest, EncodeSuccessful) {
  ...
  ASSERT_TRUE(ret_val);
  ASSERT_TRUE(rsi_command_handler.Encode(out_buffer_it, 1024) != -1);
}
...
TEST_F(EncodeTest, EncodeTimeMeasurement) {
  constexpr size_t UDP_BUFFER_SIZE = 1024;
  kuka_rsi_hw_interface::RSICommandHandler rsi_command_handler;
  char out_buffer_[UDP_BUFFER_SIZE] = {0};
  bool ret_val = true;
  ...
  timer_data.open("EncodeTest_EncodeTimeMeasurement.txt");
  ...
  for (size_t i = 0; i < 4000; i++) {
    ...
    ret_val &= (rsi_command_handler.Encode(out_buffer_it, 1024) != -1);
...
TEST_F(PugiEncodeTest, EncodeTimeMeasurement) {
  kuka_rsi_hw_interface::PugiCommandHandler pugi_command_handler(1024);
  char out_buffer_[1024] = {0};
  bool ret_val = true;
  ...
  timer_data.open("PugiEncodeTest_EncodeTimeMeasurement.txt");
  ...
  for (size_t i = 0; i < 4000; i++) {
    auto start = std::chrono::high_resolution_clock::now();
    ret_val &= (pugi_command_handler.Encode(out_buffer_, 1024) != nullptr);
    auto stop = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::nanoseconds>(stop - start);
    timer_data << i << " " << duration.count() << std::endl;
  }
