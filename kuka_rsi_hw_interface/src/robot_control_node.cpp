...
CallbackReturn RobotControlNode::on_configure(const rclcpp_lifecycle::State & previous_state)
{
  hardware_interface::HardwareInfo info;
  info.name = "KukaRSIHardwareInterface";
  info.type = "System Interface";
  info.hardware_parameters["rsi_ip_address_"] = rsi_ip_address_;
  info.hardware_parameters["rsi_port_"] = std::to_string(rsi_port_);
  info.hardware_parameters["n_dof_"] = std::to_string(n_dof_);

  kuka_rsi_hw_interface_ = std::make_unique<KukaRSIHardwareInterface>();
  kuka_rsi_hw_interface_->on_init(info);
  return SUCCESS;
}

CallbackReturn RobotControlNode::on_activate(const rclcpp_lifecycle::State & previous_state)
{
  kuka_rsi_hw_interface_->on_activate(previous_state);
...
CallbackReturn RobotControlNode::on_deactivate(const rclcpp_lifecycle::State & previous_state)
{
  kuka_rsi_hw_interface_->on_deactivate(previous_state);
...
CallbackReturn RobotControlNode::on_cleanup(const rclcpp_lifecycle::State & previous_state)
{
...
void RobotControlNode::ControlLoop()
{
  while (kuka_rsi_hw_interface_->isActive()) {
    ...
    if (kuka_rsi_hw_interface_->read(this->now(), rclcpp::Duration(0, 0)) == return_type::ERROR) {
    ...
    cv_.wait(lock);
    kuka_rsi_hw_interface_->write(this->now(), rclcpp::Duration(0, 0));
  }
}
