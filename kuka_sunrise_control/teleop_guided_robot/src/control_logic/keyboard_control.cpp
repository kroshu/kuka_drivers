    ...
    last_time_(RCL_ROS_TIME)
{
  ...
  auto callback = [this](geometry_msgs::msg::Twist::SharedPtr msg){this->messageReceivedCallback(msg);};
  key_teleop_subscription_ = this->create_subscription<geometry_msgs::msg::Twist>("key_vel", qos, callback);
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn KeyboardControl::on_configure(
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn KeyboardControl::on_cleanup(
    const rclcpp_lifecycle::State &state)
{
  (void)state;
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn KeyboardControl::on_activate(
    const rclcpp_lifecycle::State &state)
{
  (void)state;
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn KeyboardControl::on_deactivate(
    const rclcpp_lifecycle::State &state)
{
  (void)state;
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn KeyboardControl::on_shutdown(
    const rclcpp_lifecycle::State &state)
{
...
void KeyboardControl::messageReceivedCallback(geometry_msgs::msg::Twist::SharedPtr msg)
{
  ...
  if(elapsed_time > elapsed_time_treshold_){
   ...
   // RCLCPP_INFO(get_logger(), "changing_joint: %i", changing_joint_);
    ...
    double new_reference_joint_state = reference_joint_state_->position[active_joint_] + x_dir * turning_velocity_increment_;
    if(lower_limits_rad_[active_joint_]*0.9 < new_reference_joint_state && new_reference_joint_state < upper_limits_rad_[active_joint_]*0.9)
    {
...
int main(int argc, char *argv[])
{
  ...
  auto node = std::make_shared<teleop_guided_robot::KeyboardControl>("keyboard_control", rclcpp::NodeOptions());
