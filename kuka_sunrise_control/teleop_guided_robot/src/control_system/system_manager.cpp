...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn SystemManager::on_configure(
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn SystemManager::on_activate(
    const rclcpp_lifecycle::State &state)
{
  (void)state;
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn SystemManager::on_deactivate(
    const rclcpp_lifecycle::State &state)
{
  (void)state;
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn SystemManager::on_shutdown(
    const rclcpp_lifecycle::State &state)
{
  ...
  if (!changeState(CONTROL_LOGIC, lifecycle_msgs::msg::Transition::TRANSITION_UNCONFIGURED_SHUTDOWN))
  {
    return FAILURE;
  }
  if (!changeState(ROBOT_INTERFACE, lifecycle_msgs::msg::Transition::TRANSITION_UNCONFIGURED_SHUTDOWN))
  {
    return ERROR;
  }
  if (!changeState(JOINT_CONTROLLER, lifecycle_msgs::msg::Transition::TRANSITION_UNCONFIGURED_SHUTDOWN))
  {
    return ERROR;
  }
  return result;
...
{
  auto client = this->create_client<lifecycle_msgs::srv::ChangeState>(node_name + "/change_state", qos_.get_rmw_qos_profile(), cbg_);
...
int main(int argc, char *argv[])
{
  ...
  auto node = std::make_shared<teleop_guided_robot::SystemManager>("system_manager", rclcpp::NodeOptions());
