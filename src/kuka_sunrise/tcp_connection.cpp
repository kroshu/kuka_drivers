...
TCPConnection::TCPConnection(
    ...
    dataReceivedCallback_(data_received_callback), connectionLostCallback_(connection_lost_callback), socket_desc_(
        socket(AF_INET, SOCK_STREAM, 0)), cancelled_(false)
{
  if (socket_desc_ == -1)
  {
...
void TCPConnection::listen()
{
  ...
  while (!cancelled_.load())
  {
    ...
    { //TODO: is this the way to check for connection loss?
