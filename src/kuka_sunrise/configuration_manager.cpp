        ...
        std::vector<double>(7, 1000.0)), joint_damping_temp_(std::vector<double>(7, 0.7))
{
  ...
  {
    robot_manager_node_->declare_parameter("joint_stiffness",
                                           rclcpp::ParameterValue(new std::vector<double>(7, 1000.0)));
  }
  ...
  robot_manager_node_->set_on_parameters_set_callback([this](const std::vector<rclcpp::Parameter> &parameters)
  ...
  command_mode_client_ = robot_manager_node->create_client<std_srvs::srv::SetBool>("set_command_mode",
...
bool ConfigurationManager::canSetParameter(const rclcpp::Parameter &param)
{
  try
  {
    ...
    {
      RCLCPP_ERROR(robot_manager_node_->get_logger(), "Parameter %s cannot be changed while in state %s",
                   param.get_name().c_str(), robot_manager_node_->get_current_state().label().c_str());
      return false;
    }
  }
...
bool ConfigurationManager::onJointStiffnessChangeRequest(const rclcpp::Parameter &param)
{
  ...
  {
    RCLCPP_ERROR(robot_manager_node_->get_logger(), "Invalid parameter array length for parameter %s",
                 param.get_name().c_str());
    return false;
  }
