...
: LifecycleNode("robot_control"), close_requested_(false)
{
  ...
  auto command_srv_callback = [this](const std::shared_ptr<rmw_request_id_t> request_header,
      ...
      if (request->data == true) {
        response->success = this->RobotControlNode::activate(); // TODO(kovacsge11) SonarCloud suggests this is
        														// acceptable, but poor design
      } else {
        response->success = this->RobotControlNode::deactivate(); // TODO(kovacsge11) SonarCloud suggests this is
																  // acceptable, but poor design
      }
    };
