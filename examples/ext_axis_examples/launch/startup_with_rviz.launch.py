...
def generate_launch_description():
    ...
    rviz_config_launch_arg = DeclareLaunchArgument(
        ...
        default_value=os.path.join(
            get_package_share_directory("ext_axis_examples"), "config", "rviz", "view_6_axis_kl_urdf.rviz"
        ),
    )

    startup_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            [get_package_share_directory("ext_axis_examples"), "/launch/startup.launch.py"]
        )
    )
