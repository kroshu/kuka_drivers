...
1. The `rsi_gpio_joint_position.rsix` file offers an example on how to set up the different IO-s. For detailed instructions please refere to the RSI manual from KUKA Xpert.
...
1. The Etherenet RSI object requires the `rsi_gpio_ethernet.xml` configuration file.
